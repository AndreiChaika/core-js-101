...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const secondNumber = a < b ? b : a;
  return `${first}${firstNumber}, ${secondNumber}${last}`;
}
...
function getDigitalRoot(num) {
  ...
  while (string.length > 1) {
    string = string.split('').reduce((a, b) => Number(a) + Number(b)).toString();
  }
  return string;
}
...
function isBracketsBalanced(str) {
  ...
  const arrCloseBrackets = ['}', ']', ')', '>'];
  for (let i = 0; i < str.length; i += 1) {
    if (arrCloseBrackets.includes(str[i])
        && (res[res.length - 1] === arrBrackets[str[i]])) {
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    const chamber = pathes[0][i];
    let status = true;
    for (let j = 0; j < pathes.length; j += 1) {
      if (pathes[j][i] !== chamber) {
        status = false;
      }
    }
    if (status) {
